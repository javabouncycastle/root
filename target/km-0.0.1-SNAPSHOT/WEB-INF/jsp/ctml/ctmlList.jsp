                ...
                <table class="table table-bordered table-striped table-hover data-table">
                    ...
                    <c:forEach var="row" items="${certTemplates}">
	                    ...
	                    <tr id="upd_list_row_id_${row.id}" >
		                    <td>
		                    	<a href="javascript:edit('${row.id}','${row.ctmlName}','${row.kpgAlgorithm.paraValue}','${row.validity.paraValue}','${row.isValid}','${row.notes}')" class="btn btn-link">${row.id}</a>
		                    </td>
		                    <td>${row.ctmlName}</td>
		                    <td>${row.kpgAlgorithm.paraValue}</td>
		                    <td>${row.validity.paraValue}</td>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <h4 class="modal-title" >数据证书模板</h4>
            </div>
            		...
            		<div class="form">
            			<table id="id_cert_detail"  class="table table-striped table-hover table-bordered">
				             <c:if test="${success != null && message != ''}">               
				                ...
				                <div class="row">
				                  <div class="col-md-6 margin-bottom-15">
				                    <label for="kpgAlgorithm.paraValue" class="control-label">密钥类型</label>
				                    <select class="form-control margin-bottom-15" name="kpgAlgorithm.id" id="kpgAlgorithm" required="required">
				                    	<option value="">--请选择--</option>
				                    	<c:forEach var="sysCodeKpg" items="${sysCodeKpg}">
				                    		<option value="${sysCodeKpg.id}">${sysCodeKpg.paraValue}</option>
				                    	</c:forEach>
				                    </select> 
				                  </div>
				                  <div class="col-md-6 margin-bottom-15">
				                    <label for="validity.paraValue" class="control-label">有效天数</label>
				                    <select class="form-control margin-bottom-15" name="validity.id" id="validity" value="${certTemplate.validity}" required="required">
				                    	<option value="">--请选择--</option>
				                    	<c:forEach var="sysCodeVali" items="${sysCodeVali}">
				                    		<option value="${sysCodeVali.id}">${sysCodeVali.paraValue}</option>
				                    	</c:forEach>
		                   			 </select>
				                  </div>
				                </div>
...
//修改的初始化页面            
function edit(id,ctmlName,kpgAlgorithm,validity,isValid,notes){//这么只有一个参数
	debugger;
			$("#modal_update input[name='id']").val(id);
			$("#modal_update input[name='ctmlName']").val(ctmlName);
			$("#modal_update input[name='kpgAlgorithm.paraValue']").val(kpgAlgorithm);
			$("#modal_update input[name='validity.paraValue']").val(validity);
