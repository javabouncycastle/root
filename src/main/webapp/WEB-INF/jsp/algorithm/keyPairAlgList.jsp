...
<div class="col-xs-12">
	 <div class="box-body ">
	    <table id="keyPairAlgorithms_table"  class="table table-striped table-bordered table-hover">
	        <thead>
	        <tr>
...
$(function(){
	var flag = true;
	var data = ${keyPairAlgorithms};
	$("#keyPairAlgorithms_table").dataTable({
		...
		drawCallback: function( settings ){ //表格绘制完成的回调函数
			if(flag){
				 var btnText='<button class="btn btn-success btnmargin" onclick=add()><i class="fa fa-plus"></i>&nbsp;添加密钥算法</button>';					  
				     btnText = btnText +'<button class="btn  btn-warning btnmargin" onclick=edit()><i class="fa fa-pencil"></i>&nbsp;修改密钥算法</button>';    
...
function remove(id){
	...
	parent.zeroModal.confirm("确定删除该算法吗？", function(){ 
		$.ajax({
		    url:'${ctx}/algorithm/remove.do?id='+id,
		    type:'POST', //GET
...
function suspend(id){
	$.ajax({
	    url:'${ctx}/algorithm/suspend.do?id='+id,
	    type:'POST', //GET
	    async:true,    //或false,是否异步
	    dataType:'json',    //返回的数据格式：json/xml/html/script/jsonp/text
	    success:function(data){
	    ...
	    error:function(data){
	    	...
	    	location.reload();
	    	//parent.show_div(data.des);
	    }
	});
}
function activate(id){
	$.ajax({
	    url:'${ctx}/algorithm/activate.do?id='+id,
	    type:'POST', //GET
	    async:true,    //或false,是否异步
	    dataType:'json',    //返回的数据格式：json/xml/html/script/jsonp/text
	    success:function(data){
...
function add() {
	 zeroModal.show({
		title: '增加密钥算法',
		url: '${ctx}/algorithm/forWardInsert.do',
		width: '70%',
		height: '60%',
		opacity: 1,
		}); 
...
function edit() {
    ...
    zeroModal.show({
        title: '修改密钥算法',
        url: '${ctx}/algorithm/forWardUpdate.do?id='+id,
        width: '70%',
        height: '60%',
        opacity: 1
    });
