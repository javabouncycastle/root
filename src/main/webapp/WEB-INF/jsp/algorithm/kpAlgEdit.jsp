...
<div class="col-xs-12">
    ...
    <form class="form-horizontal" id="f">
        ...
        <div class="form-group">
            ...
            <div class="col-xs-6">
                <input type="text" class="form-control"  id="id" name="id" value=${keyPairAlgorithm.id} readonly="readonly">
        </div>
        </div>
        <div class="form-group">
            <label for="name" class="col-xs-3 control-label">别名</label>
            <div class="col-xs-6">
                <input type="text" class="form-control"  id="name" name="name" value=${keyPairAlgorithm.name}>
            </div>
        </div>
		<div class="form-group">
            <label for="algorithmOid" class="col-xs-3 control-label">算法OID</label>
            <div class="col-xs-6">
                <input type="text" class="form-control"  id="algorithmOid" name="algorithmOid" value="${keyPairAlgorithm.algorithmOid}">
            </div>
        </div>
       <div class="form-group">
            <label for="algorithmName" class="col-xs-3 control-label">算法英文缩写</label>
            <div class="col-xs-6">
                <input type="text" class="form-control"  id="algorithmName" name="algorithmName" value="${keyPairAlgorithm.algorithmName}">
            </div>
        </div>
		<div class="form-group">
            <label for="keysize" class="col-xs-3 control-label">密钥长度</label>
            <div class="col-xs-6">
                <input type="text" class="form-control"  id="keysize" name="keysize" value="${keyPairAlgorithm.keysize}">
            </div>
        </div>
        <div class="form-group">
...
function update(){
	...
	if(a==true){
		...
		var url = "${ctx}/algorithm/update.do";
		$.ajax({
			type : "post",
			data : data,
			url : url,
