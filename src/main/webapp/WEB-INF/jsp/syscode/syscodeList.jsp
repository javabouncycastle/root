...
$(function(){
	...
	$("#sysCodes_table").dataTable({
		...
		//data : data,
		ajax:{
			url:"/syscode/selectAll.do", //请求数据url
			typy:"post"
		},
...
function remove(id){
	...
	parent.zeroModal.confirm("确定删除该数据字典吗？", function(){ 
		$.ajax({
		    url:'${ctx}/syscode/remove.do?id='+id,
		    type:'POST', //GET
...
function suspend(id){
	$.ajax({
	    url:'${ctx}/syscode/suspend.do?id='+id,
	    type:'POST', //GET
	    async:true,    //或false,是否异步
	    dataType:'json',    //返回的数据格式：json/xml/html/script/jsonp/text
	    success:function(data){
...
function activate(id){
	$.ajax({
		url:'${ctx}/syscode/activate.do?id='+id,
	    type:'POST', //GET
	    async:true,    //或false,是否异步
	    dataType:'json',    //返回的数据格式：json/xml/html/script/jsonp/text
	    success:function(data){
...
function add() {
	 zeroModal.show({
		title: '增加数据字典',
		url: '${ctx}/syscode/forWardInsert.do',
		width: '70%',
		height: '50%',
		opacity: 1,
		okFn: function() {
